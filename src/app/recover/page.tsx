                ...
                <div className="flex flex-col gap-4 rounded-lg border-2 border-dashed bg-white p-8 hover:border-black">
                    ...
                    {success ? (
                        ...
                        <form className="flex flex-col gap-4" onSubmit={handleUsernameSubmit}>
                            ...
                            <Button type="submit" width="fit" disabled={checkUsernameMutation.status === 'pending'}>
                                Next
                            </Button>
                        </form>
                    ) : step === 1 ? (
                        <form className="flex flex-col gap-4" onSubmit={handleAnswersSubmit}>
                            ...
                            <Button type="submit" width="fit" disabled={verifyAnswersMutation.status === 'pending'}>
                                Next
                            </Button>
                        </form>
                    ) : (
                        <form className="flex flex-col gap-4" onSubmit={handlePasswordSubmit}>
                            ...
                            <Button type="submit" width="fit" disabled={updatePasswordMutation.status === 'pending'}>
                                Update Password
                            </Button>
                        </form>
                    )}
                    <div className="my-2 rounded-md border-l-4 border-blue-400 bg-blue-50 px-4 py-2 text-sm text-blue-900">
                        <strong>Note:</strong> You will be required to answer some skill testing questions about your
                    ...
                    <div className="my-2 rounded-md border-l-4 border-yellow-400 bg-yellow-50 px-4 py-2 text-sm text-yellow-900">
                        <strong>Alert: </strong>If you cannot remember your skill testing questions, you will not be
                        able to recover your account. This is a security measure to protect your data. You can try
                        contacting us at{' '}
                        <a href="mailto:support@athing.space" className="text-blue-600 hover:underline">
                            support@athing.space
                        </a>{' '}
                        for further assistance, however in most cases, we will still attempt to verify ownership and may
                        not be able to assist you if you cannot answer the questions.
                    </div>
                </div>
            </div>
        </motion.div>

...
export const Customization = () => {
    ...
    const pattern = THEME_CONFIG[String(selectedTheme) as keyof typeof THEME_CONFIG]
    ...
    return (
        ...
        >
            <div className="absolute right-5 top-5">
                <Button styles="opposite" width="fit" onClick={handleClose}>
                    Close Preview
                </Button>
            </div>
            ...
            <div className="absolute bottom-4 left-1/2 z-10 flex w-full -translate-x-1/2 flex-wrap justify-center px-2 sm:px-0">
                <div className="flex flex-row flex-wrap justify-center gap-2 sm:gap-4 max-w-full overflow-x-auto rounded-full bg-white/90 shadow-lg px-4 py-2 backdrop-blur-md">
                    {[...Array(length)].map((_, i) => {
                        const themeKey = String(i) as keyof typeof THEME_CONFIG
                        const { className: thumbClass, style: thumbStyle } = getBgClassOrImage(THEME_CONFIG[themeKey])
                        return (
                            <button
                                key={i}
                                type="button"

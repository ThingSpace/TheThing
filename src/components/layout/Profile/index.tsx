...
export interface ProfileLayoutProps {
    user: {
        ...
        styling?: number
        flaggedPosts?: { id: string; text: string; flagReason?: string }[]
        reviewPosts?: { id: string; text: string; flagReason?: string }[]
    ...
    >
        ...
        <div className="flex w-full max-w-xl flex-col gap-8 border-2 bg-white p-8 hover:border-black">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div>
                    <span className="font-semibold">User ID:</span> <span className="font-mono text-xs">{user.id}</span>
                </div>
                <div>
                ...
                <div>
                    ...
                    <span>{typeof user.styling === 'number' ? `#${user.styling}` : 'Default'}</span>
                </div>
            </div>
            ...
            {user.flaggedPosts && user.flaggedPosts.length > 0 && (
                <div className="mt-4 rounded-md border-l-4 border-yellow-400 bg-yellow-50 px-4 py-2 text-yellow-900">
                    ...
                    <ul className="list-disc ml-5 mt-2">
                        {user.flaggedPosts.map((post: { id: string; text: string; flagReason?: string }) => (
                            <li key={post.id}>
